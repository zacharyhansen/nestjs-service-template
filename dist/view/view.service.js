...
import { PGDataCellMap, PGDataFieldMap, PostgresSupportedDataTypeIdMap } from "./view.types.js";
import { Database } from "../database/database.js";
import { QueryService } from "../query/query.service.js";
import { SchematicService } from "../schematic/schematic.service.js";
export class ViewService {
    constructor(database, queryService, schematicService){
        this.database = database;
        this.queryService = queryService;
        this.schematicService = schematicService;
        this.logger = new Logger(ViewService.name);
    }
    // @UseInterceptors(LogTimerInterceptor)
    async syncViews({ schemas }) {
        await sql`select create_configuration_schema_views(ARRAY[${sql.join(schemas)}]::TEXT[]);`.execute(this.database);
        // eslint-disable-next-line sonarjs/cognitive-complexity
        await this.database.transaction().execute(async (trx)=>{
            for (const schema of schemas){
                const { schemaCacheMap, schematicViews, schematicLinks } = await this.schematicService.rootSchematic({
                    schema
                });
                // Get all views from information schema
                // role views will update automatically via cascades
                const databaseViews = await this.database.selectFrom('information_schema.views').select([
                    'table_name',
                    'table_schema',
                    'view_definition'
                ]).where('table_name', 'in', schematicViews.map((view)=>view[0].qiName)).execute();
                ...
                const columnResults = await Promise.all(databaseViews.map((view)=>this.queryService.execute({
                        query: sql`
         ...
         */ const columnResultsMap = new Map(columnResults.map((result, viewIndex)=>[
                        databaseViews[viewIndex].table_name,
                        result
                    ]));
                // Get the schema meta data for the tables/columns found via the above tableIDs
                const databaseColumns = await trx.selectFrom(`${schema}.schema_columns`).selectAll().where('table_id', 'in', Array.from(new Set(columnResults.flatMap((result)=>result.fields.map((field)=>field.tableID))))).execute();
                /**
         ...
         */ const pgColumnMap = new Map(databaseColumns.map((column)=>[
                        [
                            column.table_id,
                            column.column_id
                        ].join('_'),
                ...
                await trx.deleteFrom('configuration.view').where('name', 'not in', Array.from(databaseViewSet)).where('schema', '=', schema).execute();
                // Update or create views and their columns
                for (const databaseView of databaseViews){
                    const databaseViewColumns = columnResultsMap.get(databaseView.table_name);
                    if (!databaseViewColumns?.fields.length) {
                        throw new Error('No columns found for view - this will result in deletion of all columns for view.');
                    }
                    // Upsert the view record
                    const insertedView = await trx.insertInto('configuration.view').values(()=>{
                        const postgrestEntry = schemaCacheMap.get(databaseView.table_name);
                        return {
                            name: databaseView.table_name,
                            pg_primary_table: databaseView.table_name.replace('_p_', '').replace('_c_', ''),
                            schema,
                            updated_at: new Date(),
                    ...
                    await trx.deleteFrom('configuration.column').where('view_name', '=', insertedView.name).where('schema', '=', schema).where('name', 'not in', databaseViewColumns.fields.map((field)=>field.name)).execute();
                    // Upsert columns
                    await trx.insertInto('configuration.column').values(databaseViewColumns.fields.map((field)=>{
                        ...
                        let postgrestColumn = schemaCacheMap.get(databaseView.table_name)?.tableColumns[field.name];
                        if (SchematicService.hasUnderlyingTable(databaseView.table_name)) {
                            const underlyingColumn = schemaCacheMap.get(databaseView.table_name.replace('_p_', ''))?.tableColumns[field.name];
                            postgrestColumn = underlyingColumn ?? postgrestColumn;
                        }
                        const data_type = PostgresSupportedDataTypeIdMap[field.dataTypeID] ?? supported_pg_data_type.enum;
                        return {
                            schema,
                            ...
                            pgt_nullable: postgrestColumn?.colNullable ?? (pgColumn?.is_nullable === 'YES' ? true : false),
                            updated_at: new Date(),
                            ...
                            is_pk: schemaCacheMap.get(databaseView.table_name)?.tablePKCols.includes(postgrestColumn?.colName ?? ''),
                            // references_table: pgColumn?.references_table,
                            pgt_default: postgrestColumn?.colDefault?.toString() ?? pgColumn?.column_default?.toString()
                        };
                    })).onConflict((oc)=>oc.columns([
                            'schema',
                            'view_name',
                ...
                await trx.insertInto('configuration.link').values(schematicLinks).onConflict((oc)=>{
                    return oc.columns([
                        'schema',
                        'source_view_name',
                        'source_column_name',
                        ...
                        return {
                            ...
                            junction_source_column_name: eb.ref('excluded.junction_source_column_name'),
                            junction_target_column_name: eb.ref('excluded.junction_target_column_name')
                        };
                    });
                }).execute();
            }
        ...
        await Promise.all(schemas.map((schema)=>this.syncRoleViews({
                roleName: 'admin',
                schema
            })));
    }
   ...
   */ async syncRoleViews({ roleName, schema = 'foundation', trx, views }) {
        const database = trx ?? this.database;
        const { schematicLinks, schematicViews } = await this.schematicService.roleSchematic({
            roleName,
            schema,
            views
        });
        // Step 1: Upsert all views and delete ones not in DB
        const deleteViewsQuery = database.deleteFrom('configuration.role_view').where('schema', '=', schema).where('name', 'not in', schematicViews.map((roleView)=>roleView[0].qiName));
        if (views) {
            // if were only doing this sync for a specifc views we must filter by them
            deleteViewsQuery.where('name', 'in', views);
        }
        await Promise.all([
            database.insertInto('configuration.role_view').values(schematicViews.map((roleView)=>{
        ...
        await database.insertInto('configuration.role_column').values(schematicViews.flatMap((roleView)=>{
            ...
            return Object.keys(roleView[1].tableColumns).map((name)=>{
                return {
                    ...
                    view_name: SchematicService.rootViewFromRoleView({
                        rootView: role_view_name
                    })
                };
            });
        })).onConflict((oc)=>oc.doNothing()).returningAll().execute();
        // Step 3: delete role columns from prexisting view - break this up by view so "not in" statements dont overflow
        await Promise.all(schematicViews.map((roleView)=>{
            const role_view_name = roleView[0].qiName;
            return database.deleteFrom('configuration.role_column').where('schema', '=', schema).where('role_view_name', '=', role_view_name).where('name', 'not in', Object.keys(roleView[1].tableColumns)).execute();
        }));
        // Step 4: Upsert Links
        if (schematicLinks.length > 0) {
            await database.insertInto('configuration.role_link').values(schematicLinks).onConflict((oc)=>oc.doNothing()).returningAll().execute();
        }
    // Step 5: Delete links
    ...
    async mutateRoleViews({ columnEnabledRecords, schema = 'foundation', rootViewName }) {
        // Get the current roles in the system
        const roles = await this.database.selectFrom('configuration.role').where('name', '!=', 'admin').where('schema', '=', schema).selectAll().execute();
        ...
        for (const role of roles){
            const atLeastOneColumnEnabledForRole = columnEnabledRecords.some((record)=>Boolean(record[role.name]));
            ...
            if (atLeastOneColumnEnabledForRole) {
                const pkRecord = columnEnabledRecords.find((record)=>record.is_pk === true);
                if (pkRecord) {
            ...
            for (const record of columnEnabledRecords){
                roleColumns.set(role.name, {
                    ...
                    [record.name]: Boolean(// eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
                    record[role.name] || // Auto add primary key if any column is enabled
        ...
        await this.database.transaction().execute(async (trx)=>{
            ...
            await Promise.all(roles.map(async (role)=>{
                ...
                const columns = Object.entries(roleColumns.get(role.name) ?? [])// eslint-disable-next-line @typescript-eslint/no-unused-vars
                .filter(([_key, value])=>value).map(([key])=>key);
                await trx.schema.dropView(`${schema}.${roleViewName}`).ifExists().execute();
                // Then create it if there are columns
                // eslint-disable-next-line unicorn/prefer-ternary
                if (columns.length > 0) {
                    await trx.schema.createView(`${schema}.${roleViewName}`).orReplace().as(this.database// @ts-expect-error columns whould work
                    .selectFrom(`${schema}.${rootViewName}`)// @ts-expect-error columns whould work
                    .select(columns)).execute();
                    databaseRoleViews.push([
                        roleViewName,
                        role
                    ]);
                } else {
                    await trx // view is already gone in the DB - we also need to remove its role_view record
                    .deleteFrom('configuration.role_view').where('name', '=', SchematicService.roleViewName({
                        rootViewName,
                        role
                    })).where('schema', '=', schema).execute();
                }
            }));
        });
        ...
        if (databaseRoleViews.length > 0) {
            await Promise.all(databaseRoleViews.map((roleView)=>this.syncRoleViews({
    ...
    async viewDefinition({ viewName, schema = 'foundation' }) {
        const view = await this.database.selectFrom('information_schema.views').select([
            'table_name',
            'table_schema',
            'view_definition'
        ]).where('table_name', '=', viewName).where('table_schema', '=', schema).executeTakeFirstOrThrow();
        return view;
    }
}
ViewService = _ts_decorate([
    ...
    _ts_metadata("design:paramtypes", [
        ...
        typeof SchematicService === "undefined" ? Object : SchematicService
    ])
], ViewService);
